...
class ClientesController extends AppController
{
    ...
    public function view($id = null)
    {
        ...
        $carro=  $carros->find('list', ['limit' => 200,'keyField' => 'id',
        'valueField' =>'descripcion'])->toArray();
        ...
        if ($this->request->is('ajax')) {
            ...
            if($validador==1)
            {
                ...
                $carro_id=json_decode($this->request->data['carro']);
                $u=['carro_id'=>$carro_id,'cliente_id'=>$cliente_id];
                $union= $unions->newEntity($u, ['validate' => false]);
                $unions->save($union);
    ...
    {   if($this->Auth->user('role')=='admin'){
        ...
        if ($this->request->is('post')) {
            ...
            $carro_id=$this->request->data['carro'];
            $unions = TableRegistry::get('Unions');
            $telefonos = TableRegistry::get('Telefonos');
        ...
        $carro=  $carros->find('list', ['limit' => 200,'keyField' => 'id',
        'valueField' =>'descripcion'])->toArray();
        $this->set(compact('cliente','carro'));
        $this->set('_serialize', ['cliente']);
        }else{
