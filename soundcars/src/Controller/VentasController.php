    ...
    public function initialize()
{
    ...
    public function index()
    {
        ...
        $this->set(compact('ventatotale', 'clientes','cliente','bancos','banco','carros','carro','tengo'));
        ...
        if ($this->request->is('ajax')) {
            ...
            if($validador==1){
               ...
               $precio= $productos->find()
                    ->select(['precio','id','instalacion'])
                    ->where(['id' => $pro])
                    ->toArray();
                $preciou=$precio[0]->precio;
                $miproducto=$precio[0]->id;  
                $instal=$precio[0]->instalacion;  
                echo json_encode(compact('preciou','miproducto','instal'));
                die;
                $this->set('preciou', $preciou);
                $this->set('miproducto', $miproducto);
                 $this->set('instal', $instal);
                $this->set('_serialize', ['preciou','miproducto','instal']);
            }
            else
                {
        ...
        if ($this->request->is('post')) {
            ...
            $existencia=$productos->find()
                    ->select(['existencia','minimo','instalacion'])
                    ->where(['id' => $pro])
                    ->toArray();
            ...
            $instal=$existencia[0]->instalacion;
            if($instal==1){
                $venta = $this->Ventas->patchEntity($venta, $this->request->data);
                if ($this->Ventas->save($venta)) {
                    $this->Flash->set(__('La instalacion ha sido guardada con exito'),array('params'=>array('class'=>'alert alert-success')));
                    return $this->redirect(['action' => 'index']);
                }  else {
                    $this->Flash->error(__('la instalacion no fue guardada, por favor repita'));
                }
            }
            elseif($minimo[0]->minimo>=$restante){
                    $venta = $this->Ventas->patchEntity($venta, $this->request->data);
                if ($this->Ventas->save($venta)) {
        ...
        $tecnicos = $userTable->find('list', ['limit' => 200,'keyField' => 'id',
        'valueField' =>'username'])->where(['role'=>'tecnico'])->toArray();
       
        $this->set(compact('venta', 'producto','tecnicos'));
