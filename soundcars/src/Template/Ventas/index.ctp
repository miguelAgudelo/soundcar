...
<button type="button" class="btn btn-success btn-lg" data-toggle="modal" data-target="#myModalT">INGRESAR TECNICO
</button>                  
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">Tecnicos</h4>
      </div>
    ...
    <fieldset>
     ...
     <tr>
                <td><?php echo $this->Form->input('tecnico',['label'=>'Elija al tecnico','options'=>$tecnicos]); ?></td>
                <td><?php echo $this->Form->input('porcentaje',['label'=>'Elija el pocentaje a pagar al tecnico']); ?></td>
    ...
    <td><?= $this->Form->button('Agregar Tecnico Encargado',['id'=>'agregartecni','type'=>'button']) ?></td>
  ...
  <div class="modal-dialog" role="document">
    ...
    <fieldset>
        ...
        <?php
            ...
            echo $this->Form->input('carro',['id'=>'carron','label'=>'carro','options'=>$carros]);
        ?>
    </fieldset>
   ...
   <!-- Button trigger modal -->
<button type="button" class="btn btn-success btn-lg" data-toggle="modal" data-target="#myModalC">CREAR CARRO NUEVO
</button> 
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabel">Añadir Automovil</h4>
      </div>
      <div class="modal-body">
           <fieldset>
        ...
        ?>
         <td><?= $this->Form->button('Crear carro',['id'=>'ccarro','type'=>'button']) ?></td>
    </fieldset>
...
<script>
  $(document).ready(function () {
       ...
       consulta.done(function (data) {
         ...
         $('#agregartecni').on('click',function(){
            var t=$('#tecnico').val();
            var c=$('#cliente-id').val();
            var p=$('#producto-id').val();
            var v=$('#subtotal').val();
            var b=$('#porcentaje').val();
          
            

           $.ajax({
                data: {"user_id":t,"cliente_id" : c,"producto_id": p, "valor":v,"porcentaje":b},
                url:   'instalacions/add',
                type:  'post',
                dataType:'json', beforeSend: function (xhr) {
                        xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
                        console.log("enviado");
                ...
                success:  function (response){
                    console.log(response);
                    alert("datos para la instalcion cargados")
                                 }
                 }); 
         });
         ...
         $('#ccarro').on('click',function(){
            ...
            $.ajax({
                data: {"marca" : n,"year": c, "modelo":e,"tipo":s},
                url:   'carros/add',
                type:  'post',
                dataType:'json', beforeSend: function (xhr) {
                        xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
         ...
         $('#b').select2();
         $('#carro-id').select2();
         $('#carron').select2({ width: '500px' });
         $('#tecnico').select2({ width: '500px' });
         $('#cliente-id').select2();
